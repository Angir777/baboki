<?php
  ...
  */
  ...
  $id_product = (int) $requestURI[2];
  $action = $requestURI[3];
  if (!empty($id_product)) {$page = (string) $requestURI[1];} else {$page = '';}
      ...
      <div id="cpanel" class="col-md-12">
        <div class="card step-one">
          <div class="row card-body mb20">
            ...
            <div class="col-md-6 mt10 text-center"><h4>Magazyn</h4></div>
            <div class="col-md-3 text-right"><button class="btn btn-grey" onclick="AddProduct();"><i class="far fa-plus-square"></i> Dodaj produkt</button></div>
          </div>
        </div>
      </div>
    </div>
...
<?php 
  if (isset($action) && $action == 'edytuj') {
    echo $getcPanel->getProduct($id_product);
  } else {
    echo $getcPanel->getProducts($pagination, $page_name);
  }
?>
      </div>
    </div> 
